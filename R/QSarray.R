...
newQSarray = function(obj=NULL,
  ...
  reqField = c("mean","SD","dof")
  optField = c("sd.alpha","var.method","labels","contrast","pairVector","pathways","path.mean","path.PDF",
               "path.size","ranges","n.points","vif","absolute.p","homogeneity")
...
qsTable = function(QSarray, number=20, sort.by=c("fdr","p","logFC","none")){
  ...
  if(!is.na(sort.by)){
    results = results[order(results[,sort.by]),]
  }
