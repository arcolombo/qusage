...
The qusage package can be downloaded from the Bioconductor website using the "biocLite()" command. This will download and install the qusage package along with any package dependenies not already installed. 
...
To demonstrate this, we will use MsigDB's KEGG database (which can be downloaded at \url{www.broadinstitute.org/gsea/msigdb}). The database has been loaded into R and stored as a list of character vectors in the \texttt{MSIG.geneSets} object. With this object, we run \texttt{qusage} exactly as before.
...
<<plotCIsGenes,fig=TRUE,include=FALSE,echo=FALSE,width=11,height=6>>=
par(mar=c(8,4,1,2))
...
Because this function is simply an extension of the basic \texttt{plot} function, these plots can be heavily customized. This customization can be accomplished both through the basic plotting parameters, such as \texttt{xlab, ylab, main}, and the parameters passed on to \texttt{par}, and via some parameters which are specific to \texttt{plotDensityCurves}. The function-specific parameters are listed below.
